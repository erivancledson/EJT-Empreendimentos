...
public class TelaCadastroEncanador {

	
	public JFrame frmCadastroEncanador;
	...
	private EJT.Encanador.Encanador encanador;
	...
	private void initialize() {
		frmCadastroEncanador = new JFrame();
		frmCadastroEncanador.setType(Type.UTILITY);
		frmCadastroEncanador.getContentPane().setBackground(Color.WHITE);
		frmCadastroEncanador.setTitle("CADASTRO ENCANADOR");
		frmCadastroEncanador.setBounds(100, 100, 630, 426);
		frmCadastroEncanador.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
		frmCadastroEncanador.getContentPane().setLayout(null);
		
		frmCadastroEncanador.setUndecorated(true);
	    frmCadastroEncanador.setLocationRelativeTo(null); 
		
		JPanel panelBotoes = new JPanel();
		panelBotoes.setBackground(Color.WHITE);
		panelBotoes.setBounds(10, 337, 594, 41);
		frmCadastroEncanador.getContentPane().add(panelBotoes);
		panelBotoes.setLayout(null);
		
		JButton btnLimpar = new JButton("LIMPAR");
		btnLimpar.setForeground(SystemColor.activeCaption);
		btnLimpar.setBackground(SystemColor.inactiveCaptionBorder);
		btnLimpar.setIcon(new ImageIcon(TelaCadastroEncanador.class.getResource("/EJT/imagens/buttonLimpar.png")));
		...
		btnCancelar.setIcon(new ImageIcon(TelaCadastroEncanador.class.getResource("/EJT/imagens/buttonCancelar.png")));
		btnCancelar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
			
				frmCadastroEncanador.dispose();
		...
		btnCadastrar.setIcon(new ImageIcon(TelaCadastroEncanador.class.getResource("/EJT/imagens/buttonComfirma.png")));
		...
		frmCadastroEncanador.getContentPane().add(panel);
		...
		JLabel lblDadosPessoais = new JLabel("Cadastro Encanador");
		lblDadosPessoais.setBounds(241, 11, 165, 14);
		frmCadastroEncanador.getContentPane().add(lblDadosPessoais);
		...
		lblSair.setIcon(new ImageIcon(TelaCadastroEncanador.class.getResource("/EJT/imagens/botao_fechar.png")));
	...
	private void cadastrarEletricista() throws Exception {
		...
		Encanador encanador = new Encanador(nome, cpf, rg, disponibilidade, endereco, contato);
		
		try {
	
		Fachada.getInstance().encanadorCadastrar(encanador);
		JOptionPane.showMessageDialog(frmCadastroEncanador, "Cadastro Realizado com sucesso!");
		this.limparCampos();
		
		} catch (CPFInvalidoException e) {
			JOptionPane.showMessageDialog(frmCadastroEncanador, e.getMessage(), " ERRO ", JOptionPane.ERROR_MESSAGE);
		} catch (EncanadorCadastradoException e){
			JOptionPane.showMessageDialog(frmCadastroEncanador, e.getMessage(), " ERRO ", JOptionPane.ERROR_MESSAGE);
